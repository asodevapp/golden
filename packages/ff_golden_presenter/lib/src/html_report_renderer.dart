...
final class HtmlReportRenderer {
  ...
  GoldenPresenterResult render(GoldenCatalog catalog) {
    final socialDescription =
        'Browse ${catalog.imageCount} golden test ${catalog.imageCount == 1 ? 'image' : 'images'} '
        'across ${catalog.scenarios.length} ${catalog.scenarios.length == 1 ? 'scenario' : 'scenarios'} '
        'in a searchable FF Golden Presenter report.';
    final document = StringBuffer()
      ...
      ..writeln(
        '  <meta name="description" content="${_escapeAttribute(socialDescription)}">',
      )
      ...
      ..writeln(
        '  <meta property="og:title" content="${_escapeAttribute(title)}">',
      )
      ..writeln(
        '  <meta property="og:description" content="${_escapeAttribute(socialDescription)}">',
      )
      ..writeln('  <meta name="twitter:card" content="summary">')
      ..writeln(
        '  <meta name="twitter:title" content="${_escapeAttribute(title)}">',
      )
      ..writeln(
...
<section class="controls" aria-label="Golden image filters">
  <div class="controls__content">
    ...
    ${_selectIfPresent('device', 'Device', catalog.devices)}
    ${_selectIfPresent('theme', 'Theme', catalog.themes)}
    ${_selectIfPresent('locale', 'Locale', catalog.locales)}
  ...
  String _select(String id, String label, List<String> values) {
    ...
    return '''
<div class="select-field" data-custom-select>
  <span class="select-field__label" id="$id-filter-label">$label</span>
  <select id="$id-filter" aria-labelledby="$id-filter-label">$options</select>
  <button class="select-trigger" type="button" aria-haspopup="listbox" aria-expanded="false" aria-controls="$id-filter-options" aria-labelledby="$id-filter-label $id-filter-value">
    <span id="$id-filter-value" data-select-value>${_escape('All $plural')}</span>
    <span class="select-trigger__chevron" aria-hidden="true"></span>
  </button>
  <div class="select-popover" id="$id-filter-options" role="listbox" aria-labelledby="$id-filter-label" hidden></div>
</div>''';
  }
...
<section class="empty-state" id="empty-state"$hidden>
  ...
  String _renderFooter() {
    const presenterAttribution =
        'Generated by <a href="https://github.com/asodevapp/golden/tree/master/packages/ff_golden_presenter">FF Golden Presenter</a>';
    if (!showSupportAttribution) {
      return '<footer>$presenterAttribution</footer>';
    }
    return '''
<footer>
  Made by the <a href="https://aso.dev/?utm_source=ff_golden&amp;utm_medium=referral" rel="sponsored noopener" target="_blank">ASO.dev team</a>
...
const _styles = r'''
  <style>
    :root {
      ...
      --accent-soft: rgba(142, 227, 197, .12);
      --menu-shadow: 0 18px 46px rgba(0, 0, 0, .38);
    ...
    :root[data-theme="light"] {
      ...
      --accent-soft: rgba(19, 122, 90, .1);
      --menu-shadow: 0 18px 42px rgba(37, 49, 72, .2);
    }
    ...
    * { box-sizing: border-box; }
    ...
    .select-trigger:hover { border-color: color-mix(in srgb, var(--accent) 70%, var(--border)); }
    .select-field.is-open .select-trigger { border-color: var(--accent); box-shadow: 0 0 0 3px var(--accent-soft); }
    .select-trigger [data-select-value] { min-width: 0; overflow: hidden; color: var(--text); font-size: 13px; font-weight: 650; letter-spacing: 0; text-overflow: ellipsis; text-transform: none; white-space: nowrap; }
    .select-trigger__chevron { width: 8px; height: 8px; flex: none; border-right: 2px solid var(--muted); border-bottom: 2px solid var(--muted); transform: translateY(-2px) rotate(45deg); transition: transform .16s ease; }
    .select-field.is-open .select-trigger__chevron { transform: translateY(2px) rotate(225deg); }
    .select-field.is-enhanced .select-popover { position: absolute; top: calc(100% + 8px); left: 0; z-index: 40; display: grid; width: 100%; min-width: 160px; max-height: min(290px, 55vh); gap: 3px; overflow: auto; padding: 6px; border: 1px solid var(--border); border-radius: 14px; background: var(--panel); box-shadow: var(--menu-shadow); color: var(--text); letter-spacing: 0; text-transform: none; }
    .select-option { position: relative; width: 100%; min-height: 38px; padding: 8px 34px 8px 10px; border: 0; border-radius: 9px; background: transparent; color: var(--muted); text-align: left; font-size: 13px; font-weight: 650; line-height: 1.3; }
    .select-option:hover, .select-option:focus-visible { outline: none; background: var(--panel-soft); color: var(--text); }
    .select-option[aria-selected="true"] { background: var(--accent-soft); color: var(--accent); }
    .select-option[hidden] { display: none; }
    .select-option::after { position: absolute; top: 50%; right: 11px; content: "✓"; color: var(--accent); font-size: 15px; opacity: 0; transform: translateY(-50%); }
    .select-option[aria-selected="true"]::after { opacity: 1; }
...
<script>
  (() => {
    ...
    function cardMatches(card, query, excludedFilter = null) {
      return (!query || card.dataset.search.includes(query)) &&
        filters.every(([key, control]) =>
          key === excludedFilter || !control.value || card.dataset[key] === control.value);
    }

    function updateFilterOptions(query) {
      filters.forEach(([key, control]) => {
        const available = new Set(
          cards
            .filter((card) => cardMatches(card, query, key))
            .map((card) => card.dataset[key])
            .filter(Boolean),
        );
        const wrapper = control.closest('[data-custom-select]');
        const enhancedOptions = wrapper ? customOptions(wrapper) : [];
        Array.from(control.options).forEach((option, index) => {
          const applicable = !option.value ||
          ...
          if (enhancedOptions[index]) {
            enhancedOptions[index].disabled = !applicable;
            enhancedOptions[index].hidden = !applicable;
          }
        });
      });
    }
    ...
    function customOptions(wrapper, visibleOnly = false) {
      const options = Array.from(wrapper.querySelectorAll('.select-option'));
      return visibleOnly ? options.filter((option) => !option.hidden) : options;
    }

    function syncCustomSelect(wrapper) {
      const select = wrapper.querySelector('select');
      const selected = select.options[select.selectedIndex];
      wrapper.querySelector('[data-select-value]').textContent = selected.textContent;
      customOptions(wrapper).forEach((option) => {
        option.setAttribute(
          'aria-selected',
    ...
    function closeCustomSelect(wrapper, restoreFocus = false) {
      ...
      wrapper.querySelector('.select-popover').hidden = true;
      trigger.setAttribute('aria-expanded', 'false');
      if (openSelect === wrapper) openSelect = null;
      if (restoreFocus) trigger.focus();
    }

    function openCustomSelect(wrapper, edge = null) {
      if (openSelect && openSelect !== wrapper) closeCustomSelect(openSelect);
      const trigger = wrapper.querySelector('.select-trigger');
      const popover = wrapper.querySelector('.select-popover');
      const options = customOptions(wrapper, true);
      ...
      const target = edge === 'last' ? options[options.length - 1] : selected || options[0];
      target?.focus();
    }
    ...
    customSelects.forEach((wrapper) => {
      ...
      Array.from(select.options).forEach((nativeOption, index) => {
        ...
        option.addEventListener('keydown', (event) => {
          ...
          let target = null;
          if (event.key === 'ArrowDown') target = options[(current + 1) % options.length];
      ...
      trigger.addEventListener('keydown', (event) => {
        ...
        event.preventDefault();
        openCustomSelect(wrapper, event.key === 'ArrowUp' ? 'last' : null);
      });
    });
    ...
    document.addEventListener('pointerdown', (event) => {
      if (openSelect && !openSelect.contains(event.target)) closeCustomSelect(openSelect);
    });
